...
* https://issues.redhat.com/browse/OSSM-4851[OSSM-4851] Previously, when runAsGroup, runAsUser, or fsGroup were nil this was causing an error in the operator deploying new pods in a namespace scoped inside the mesh. To avoid this we add a yaml validation to avoid the nil value
