...
[id="service-deployment-model_{context}"]
== Service deployment model
When using the operator with `FlowCollector` `spec.deploymentModel` set to `Service`, agents are deployed per node (as `DaemonSets`), and `flowlogs-pipeline` is a standard `Deployment with a Service`. The `flowlogs-pipeline` can be scaled using `spec.processor.consumerReplicas`.

Note that Loki isn't managed by the Network Observability Operator and must be installed separately using the {lok-op}. Same goes with Prometheus and any custom receiver.

[id="direct-deployment-model_{context}"]
== Direct deployment model
When using the operator with `FlowCollector` `spec.deploymentModel` set to `Direct`, agents and the `flowlogs-pipeline` are both deployed per node (as `DaemonSets). This is perfect for an assessment of the technology, suitable on small clusters, but isn't very memory efficient in large clusters as every instance of `flowlogs-pipeline` ends up caching the same cluster information, which can be huge.

Like in other modes, data stores aren't managed by the Network Observability Operator.


[id="kafak-deployment-model_{context}"]
